...
def compute_chamfer_meshes(gt:trimesh.Trimesh, pred: trimesh.Trimesh, num_points=1000000, norm=2):
    ...
    max_dist = 1000 # 0.5
    mask1 = dist_gt_pred < max_dist
    mask2 = dist_pred_gt < max_dist
    chamf_gt_pred = np.mean(dist_gt_pred[mask1])
    chamf_pred_gt = np.mean(dist_pred_gt[mask2])

    return {
        'accuracy': chamf_pred_gt,
        'completeness': chamf_gt_pred,
        'chamfer*100': 100 * (chamf_gt_pred + chamf_pred_gt) / 2
    }
...
def filter_mesh(mesh, cams, resolution, masks):
    ...
    meshes = Meshes(verts=[vertices],
                    faces=[faces])
    with torch.no_grad():
        for i in tqdm(range(100), desc="Rasterization"):
            if i % 1 == 0: # select img for mesh postprocess, use 1/2 for rasterizing speed
