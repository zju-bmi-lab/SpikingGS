	...
	bool prefiltered)
{
	...
	glm::vec3 normal = glm::vec3(n_view.x, n_view.y, n_view.z);
	...
	Jinv_u0_u1[10] = 0;
	if (grazing) Jinv_u0_u1[10] = 1;
	for (int i = 0; i < 11; i++) Jinv[idx * 11 + i] = Jinv_u0_u1[i];
	...
	bool record_transmittance)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float normal[3] = {collected_normal[j].x, collected_normal[j].y, collected_normal[j].z};
			...
			if(Jinv_temp[10]==0) depth -= pos_dif.z;
			if (depth < NEAR_PLANE) continue;
			
			const float G1 = G * out;
			float alpha = min(0.99f, con_o.w * G1);
