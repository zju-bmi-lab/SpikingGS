	...
	const bool debug)
{
  ...
  torch::Tensor ray = torch::full({50*2, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		v_th.contiguous().data<float>(), 
		out_others.contiguous().data<float>(),
		ray.contiguous().data<float>(),
...
//   printf("rend%d", radii[1]);
  return std::make_tuple(rendered, out_color, out_depth, out_others, ray, radii, geomBuffer, binningBuffer, imgBuffer, transmittance, num_occluder);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dvth = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dvth_added = torch::zeros({1, 1}, means3D.options());
  ...
  {  
	  ...
	  v_th.contiguous().data<float>(),
	  dL_dvth.contiguous().data<float>(),
	  dL_dz.contiguous().data<float>(),
	  debug);
  }
//   dL_dvth_added[0][0] = torch::sum(dL_dvth);
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dvth);
}
